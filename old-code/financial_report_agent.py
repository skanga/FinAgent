"""
financial_report_agent.py - Refactored Version
...
class PDFConverter:
    ...
    def markdown_to_pdf(markdown_content: str, output_path: str, chart_paths: List[str] = None) -> None:
        ...
        try:
            ...
            from reportlab.platypus import Image as RLImage, Table, TableStyle, KeepTogether
            ...
            # Custom styles with better encoding support
            ...
            # Create a mapping of chart filenames to full paths
            if chart_paths:
                for chart_path in chart_paths:
                    chart_name = Path(chart_path).name
                    chart_paths_dict[chart_name] = chart_path
                    logger.info(f"Chart available for embedding: {chart_name} -> {chart_path}")
            
            # Helper function to clean and format text for PDF
            def clean_text_for_pdf(text: str) -> str:
                """Clean text and convert markdown to HTML for ReportLab."""
                # First, handle special unicode characters that cause issues
                replacements = {
                    '\u2022': '-',  # bullet
                    '\u2013': '-',  # en dash
                    '\u2014': '-',  # em dash
                ...
                # Code: `text` -> text (just remove backticks, or use courier)
            ...
            while i < len(lines):
                ...
                # Skip horizontal rules and empty "Visualizations" sections
                ...
                # Check if this is the "Visualizations" header
                if line.strip() in ['4. Visualizations', '## Visualizations', '### Visualizations']:
                    # Add the header
                    clean_line = clean_text_for_pdf(line.replace('#', '').strip())
                    p = Paragraph(clean_line, heading2_style)
                    story.append(p)
                    story.append(Spacer(1, 0.15*inch))
                    
                    # Now embed ALL charts
                    for chart_name, chart_path in chart_paths_dict.items():
                        if Path(chart_path).exists() and chart_path not in charts_added:
                            try:
                                # Add ticker name as caption
                                ticker = chart_name.replace('_close.png', '').upper()
                                caption = Paragraph(f"<b>{ticker} Price History</b>", body_style)
                                story.append(caption)
                                story.append(Spacer(1, 0.05*inch))
                                
                                # Add chart image
                                img = RLImage(chart_path, width=5.5*inch, height=3.2*inch)
                                img.hAlign = 'CENTER'
                                story.append(img)
                ...
                # Check for individual chart references in text and skip them
                is_chart_reference = False
                for chart_name in chart_paths_dict.keys():
                    if chart_name in line:
                        is_chart_reference = True
                        break
                
                if is_chart_reference:
                    i += 1
                    continue
                ...
                # Headers
                if line.startswith('# '):
                    text = line_clean[2:].replace('<b>', '').replace('</b>', '').strip()
                    p = Paragraph(text, title_style)
                    story.append(p)
                    story.append(Spacer(1, 0.15*inch))
                elif line.startswith('## '):
                    text = line_clean[3:].replace('<b>', '').replace('</b>', '').strip()
                    p = Paragraph(text, heading2_style)
                    story.append(p)
                    story.append(Spacer(1, 0.12*inch))
                elif line.startswith('### '):
                    text = line_clean[4:].replace('<b>', '').replace('</b>', '').strip()
                    p = Paragraph(text, heading3_style)
                    story.append(p)
                    story.append(Spacer(1, 0.1*inch))
                # Numbered sections (e.g., "1. Executive Summary")
                ...
                # Lists
                    ...
                    text = line_clean[prefix_len:].strip()
                ...
                # Tables
                    ...
                    while j < len(lines) and lines[j].strip().startswith('|'):
                        ...
                        if not row.startswith('|---') and not row.startswith('| ---'):
                            cells = [clean_text_for_pdf(c.strip()).replace('<b>', '').replace('</b>', '').replace('<i>', '').replace('</i>', '') 
                ...
                # Regular paragraph
                    ...
                    if line_clean.strip():
                        p = Paragraph(line_clean, body_style)
                        story.append(p)
            ...
            # Build PDF
            doc.build(story)
            logger.info(f"PDF created successfully with {len(charts_added)} embedded charts")
            
            if len(charts_added) == 0 and chart_paths:
                logger.warning("No charts were embedded despite charts being available!")
