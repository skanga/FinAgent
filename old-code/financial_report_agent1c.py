...
Fixed version addressing threading issues, datetime warnings, and report quality problems.
"""
...
Check for these specific issues and return a JSON with "issues" as a list of individual issues:
...
For each issue found, provide a clear, concise description as a separate string in the list.

Example of expected output:
{{
  "issues": [
    "Missing explanation for volatility metric",
    "Failed to mention GOOG data incompleteness",
    "Referenced missing comparison chart"
  ],
  "revised": "Revised markdown content here...",
  "score": 7
}}
...
Return ONLY the JSON object, no other text."""
    ...
    def review_report(self, draft: str, data_summary: Dict, charts_available: List[str]) -> tuple[List[str], str]:
        """Enhanced review with chart validation and proper issue parsing."""
        ...
        try:
            ...
            # Fix: Ensure issues is always a list and properly formatted
            if isinstance(issues, str):
                # Split string by newlines and clean up
                issues = [issue.strip() for issue in issues.split('\n') if issue.strip()]
            
            # Clean up issue formatting (remove numbers, bullets, etc.)
            cleaned_issues = []
            for issue in issues:
            ...
            logger.info(f"Review found {len(cleaned_issues)} issues, score: {review_json.get('score', 'N/A')}")
            return cleaned_issues, revised
...
class FixedFinancialReportOrchestrator:
    ...
    def run(self, user_request: str, output_dir: str = "./report_out_fixed") -> ReportMetadata:
        ...
        # Simple request parsing - extract tickers from request
        tickers = self._extract_tickers_from_request(user_request)
        period = self._extract_period_from_request(user_request)
    ...
    def _extract_tickers_from_request(self, user_request: str) -> List[str]:
        """Extract ticker symbols from user request."""
        ...
        for word in words:
            ...
            if 2 <= len(clean_word) <= 5 and clean_word.isalpha():
                ...
                if clean_word in common_tickers.values():
                    tickers.append(clean_word)
                elif len(clean_word) <= 4:  # Assume short uppercase words are tickers
                    tickers.append(clean_word)
        
        # If no tickers found, use defaults
        if not tickers:
            tickers = ['AAPL', 'MSFT', 'GOOG']
        
        # Remove duplicates and limit
        return list(dict.fromkeys(tickers))[:10]  # Max 10 tickers
    
    def _extract_period_from_request(self, user_request: str) -> str:
        """Extract time period from user request."""
        ...
        period_mappings = {
            ...
            'quarter': '3mo', 'quarterly': '3mo',
            'year': '1y', 'yearly': '1y', 'annual': '1y',
            '2 year': '2y', 'two year': '2y',
            '5 year': '5y', 'five year': '5y',
            '10 year': '10y', 'ten year': '10y',
...
def setup_cli_arguments():
    ...
    parser = argparse.ArgumentParser(description='Generate financial reports with risk analysis')
    parser.add_argument(
        '--request', 
        '-r',
        type=str,
        default="Generate financial report for AAPL, MSFT, and GOOG with risk analysis",
        help='User request for financial analysis (e.g., "Analyze TSLA and NVDA for past 6 months")'
    )
    parser.add_argument(
        '--output-dir',
        '-o', 
        type=str,
        default="./report_out_fixed",
    ...
    parser.add_argument(
        ...
        '-t',
        type=str,
        help='Comma-separated list of tickers to analyze (overrides request parsing)'
    )
    parser.add_argument(
        '--period',
        '-p',
        ...
        help='Time period for analysis (overrides request parsing)'
    )
    return parser

# ---------- Main Entry Point ----------
def main():
    """Main entry point for fixed version with CLI support."""
    ...
    try:
        ...
        # Override request if tickers are provided via CLI
        user_request = args.request
        if args.tickers:
            tickers = [t.strip().upper() for t in args.tickers.split(',')]
            period = args.period or '1y'
            user_request = f"Generate financial report for {', '.join(tickers)} for the past {period} with risk analysis"
            logger.info(f"Using CLI provided tickers: {tickers}, period: {period}")
        
        # Run fixed analysis
        result = orchestrator.run(user_request, output_dir=args.output_dir)
