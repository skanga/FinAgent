...
class PDFConverter:
    ...
    def markdown_to_pdf(markdown_content: str, output_path: str, chart_paths: List[str] = None) -> None:
        ...
        try:
            ...
            # Custom style for better body text
            ...
            def clean_markdown_for_pdf(text: str) -> str:
                """Remove markdown syntax and replace special chars."""
                ...
                # Convert markdown code to courier
            ...
            while i < len(lines):
                ...
                # Check for chart references FIRST
                chart_found = False
                if chart_paths_set:
                    for chart_path in chart_paths_set:
                        chart_name = Path(chart_path).name
                        # Look for chart reference (e.g., AAPL_close.png)
                        if chart_name in line and chart_path not in charts_added:
                            # Skip the text reference line entirely
                            # Embed chart image directly
                            if Path(chart_path).exists():
                                try:
                                    img = RLImage(chart_path, width=5.5*inch, height=3.3*inch)
                                    # Center the image
                                    img.hAlign = 'CENTER'
                                    story.append(img)
                                    story.append(Spacer(1, 0.2*inch))
                                    charts_added.add(chart_path)
                                    logger.info(f"Embedded chart in PDF: {chart_name}")
                                except Exception as e:
                ...
                # Clean the line
                line_clean = clean_markdown_for_pdf(line)
                
                # Headers
                if line.startswith('# '):
                    text = line_clean[2:].replace('<b>', '').replace('</b>', '')
                    p = Paragraph(text, styles['Heading1'])
                    story.append(p)
                    story.append(Spacer(1, 0.2*inch))
                elif line.startswith('## '):
                    text = line_clean[3:].replace('<b>', '').replace('</b>', '')
                    p = Paragraph(text, styles['Heading2'])
                    story.append(p)
                    story.append(Spacer(1, 0.15*inch))
                elif line.startswith('### '):
                    text = line_clean[4:].replace('<b>', '').replace('</b>', '')
                ...
                # Tables
                    ...
                    while i < len(lines) and lines[i].strip().startswith('|'):
                        row = lines[i].strip()
                        if not row.startswith('|---'):  # Skip separator
                            cells = [clean_markdown_for_pdf(c.strip()).replace('<b>', '').replace('</b>', '') 
            ...
            # Build PDF
            doc.build(story)
            logger.info(f"PDF created successfully with {len(charts_added)} charts: {output_path}")
